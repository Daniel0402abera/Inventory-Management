...
function DisplayProductCount() {
  const { data, isLoading:isLoadingAll } = useGet(`${baseURL}shelves/products-count`);
  ...
  const handleGet = () => {
    const fetchData = async () => {
        clearError()
        try {
          const products = await get(`shelves/products-count/${formik.values.productID}`); // 'products' is the endpoint you want to fetch data from
  ...
  return (
    <div>
      ...
      >
        <Paper elevation={3}>
          <Typography variant="h6">Display Product Count on the shelf </Typography>
          <MaterialReactTable
        ...
        <Paper elevation={3}>
          <Typography> Display Product products on the shelf by id</Typography>
...
export default DisplayProductCount;
